...
#define AUTH_NOT_REQUIRED         0x00
#define AUTH_METHOD_NOT_SUPPORTED 0xFF

/* Команды */
#define TCP_ESTABLISH_CONNECTION 0x01

/* Типы адреса */
#define IPV4_ADDRESS     0x01
#define HOSTNAME_ADDRESS 0x03
#define IPV6_ADDRESS     0x04

/* Типы ответов на команды */
#define REQUEST_GRANTED            0x00
...
typedef struct socks_state {
  int state;
  bool found_auth;
  size_t len; // Количество оставшихся типов аутентификации или текущая прочитанная длина адреса
  uint8_t addr_type;
  uint16_t port;
  char address[MAX_HOSTNAME_LEN + 1];
} socks_state_t;
...
static bool handle_data_forward(size_t pos) {
